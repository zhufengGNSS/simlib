...
// basic operating system test
      ...
      defined(__WIN32__)||defined(__FreeBSD__)||defined(__sun__))
...
const char *Process::Name() const
{
    ...
    if (*name)
        ...
        return SIMLIB_create_name("Process#%lu", _Ident);       // generic
}
...
////////////////////////////////////////////////////////////////////////////
/// Activate process now
void Process::Activate()
{
    Entity::Activate(); // schedule now


// BUG: virtual call to Process::Activate(Time)   ####
#if 0
    if (!isCurrent())
        return;
    // This is used for example in Barrier.Enter() for last process
    THREAD_INTERRUPT();
#endif
}
...
void Process::_Run() throw() // no exceptions
{
    ...
    volatile int mylocal = CANARY1;     // should be automatic = on stack
